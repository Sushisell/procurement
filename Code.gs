...
function submitOrder(payload) {
  ...
  if (!branch || !branch.url) {
    throw new Error('Не найдена ссылка на таблицу филиала: ' + payload.branch);
  }

  const date = parseDate_(payload.date);
  const branchSpreadsheet = SpreadsheetApp.openByUrl(branch.url);
...
function readCatalog_(spreadsheet) {
  ...
  const knownSuppliers = getKnownSuppliers_(spreadsheet);
  const fromTemplate = templateSheet ? readTemplateProducts_(templateSheet, knownSuppliers) : [];

  if (fromTemplate.length) {
    return groupProductsBySupplier_(fromTemplate);
  }

  if (!pricesSheet) return [];
  const values = pricesSheet.getDataRange().getDisplayValues();
  const rows = values.slice(1).map((row) => ({
    supplier: clean_(row[0]),
    product: clean_(row[1]),
    price: toNumber_(row[2]),
    unit: clean_(row[3]),
  })).filter((item) => item.supplier && item.product);

  return groupProductsBySupplier_(rows);
}
